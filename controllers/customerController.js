...
exports.login = async(req, res) => {
  ...
  db.query('SELECT * FROM customer WHERE Email = ? AND MobileNumber = ?', [email, mobile], function(err, rows) {
    if (!err) {
      if (rows.length > 0) {
          res.send({
              "code": 200,
...
exports.register = async(req, res) => {
  ...
  db.query("SELECT * FROM customer WHERE email = ?", [email], function(err, rows) {
    if (!err) {
      if (rows.length > 0) {
        res.send({
          "code": 400,
        ...
        db.query("INSERT INTO customer (Name, Email, MobileNumber, CreatedDate) VALUES (?, ?, ?, NOW())", [name, email, mobile], function(err, rows) {
...
exports.getCustomer = async(req, res) => {
  ...
  db.query("SELECT * FROM customer WHERE CustomerId = ?", [customerId], function(err, rows) {
    if (!err) {
      if (rows.length > 0) {
        res.send({
          "code": 200,
